...
export function createWorkspaceTemplate(options: CreateWorkspaceTemplateOptions): string {
  ...
  return `vscode://metyatech.workspace-launch-by-link/start?${params.toString()}`;
}
...
function ensureTemplateMaterialized(spec: TemplateSpec): void {
  ...
  console.log(`[WorkspaceTemplate] Materialized template "${spec.workspaceId}" at ${relativePath}`);
}
